  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="GST"
        tabs={
          <div className="flex gap-1 overflow-x-auto rounded-lg bg-slate-100 p-1 dark:bg-[#16161f]">
            {tabs.map((t) => (
              <button
                key={t.key}
                ...
                className={`whitespace-nowrap rounded-md px-4 py-2 text-sm font-medium transition ${
                  tab === t.key
                    ? "bg-white text-slate-900 shadow-sm dark:bg-[#1a1a24] dark:text-[#f1f5f9]"
                    : "text-slate-600 hover:text-slate-900 dark:text-[#94a3b8] dark:hover:text-[#f1f5f9]"
                }`}
              >
                {t.label}
              </button>
