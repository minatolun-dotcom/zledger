...
interface ChainStatus {
  total: number;
  ok: boolean;
  breaks: { id: string; created_at: string | null; reason: string }[];
...
export default function AuditLogPage() {
  ...
  useEffect(() => {
    api.get<ChainStatus>("/audit/chain/verify")
      .then(setChain)
      .catch(() => setChain(null))
      .finally(() => setChainChecking(false));
  }, []);
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          {!chainChecking && chain && (
            chain.ok ? (
              <span
                className="inline-flex items-center gap-1.5 rounded-full border border-emerald-200 dark:border-emerald-500/30 bg-emerald-50 dark:bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-700 dark:text-emerald-400"
                title="Every entry's hash links correctly to its predecessor — the log is tamper-evident"
              >
                <svg className="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                </svg>
                Chain verified · {chain.total} entries
              </span>
            ) : (
              <span
                className="inline-flex items-center gap-1.5 rounded-full border border-red-300 dark:border-red-500/40 bg-red-50 dark:bg-red-500/10 px-2.5 py-1 text-xs font-medium text-red-700 dark:text-red-400"
                title={(chain.breaks || []).map((b) => `${b.id}: ${b.reason}`).join("\n") || "Chain verification failed"}
              >
                <svg className="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                </svg>
                Chain broken · {(chain.breaks || []).length} tampered
              </span>
            )
          )}
