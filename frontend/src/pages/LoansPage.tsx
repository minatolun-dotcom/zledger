...
type Tab = "given" | "taken" | "advances" | "summary";  const fmt = (n: number) => n.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
          ...
          <div className="mt-2">
            {loading ? (
              ...
              <SortableTable
                ...
                actions={(loan) => {
                  const acts: Array<{ icon: ReactNode; label: string; onClick: () => void; danger?: boolean }> = [];
                  if (loan.status !== "closed") acts.push({ icon: payIcon, label: "Pay", onClick: () => openPaymentModal(loan) });
                  acts.push({ icon: editIcon, label: "Edit", onClick: () => openEditModal(loan) });
