      ...
      <div className="w-full max-w-lg rounded-2xl bg-white p-8 shadow-xl ring-1 ring-slate-200">
          ...
          <div className="mt-6 space-y-2">
            {companies.map((co) => (
              <button key={co.id} onClick={() => handleSelect(co.id)}
                className="w-full rounded-lg border border-slate-200 px-4 py-3 text-left hover:border-brand-600 hover:bg-brand-50">
                <span className="font-medium text-slate-900">{co.name}</span>
                <span className="ml-2 rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600">{co.role}</span>
              </button>
            ))}
          </div>
        )}
        ...
        {!showCreate ? (
          <button onClick={() => setShowCreate(true)}
            className="mt-6 w-full rounded-lg border-2 border-dashed border-slate-300 py-3 text-sm font-medium text-slate-600 hover:border-brand-600 hover:text-brand-600">
            + Create new company
          </button>
        ) : (
          <form onSubmit={handleCreate} className="mt-6 space-y-4">
            ...
            <div className="border-t border-slate-200 pt-4">
              <h3 className="text-sm font-semibold text-slate-700">Financial Year *</h3>
              <p className="mt-0.5 text-xs text-slate-500">Select the start date — end date is auto-calculated.</p>
              ...
              {fyStart && fyEnd && (
                <p className="mt-2 text-xs text-slate-500">FY Name: {generateFyName(fyStart)}</p>
              )}
            </div>
