          ...
          {showHsnForm && (
            <div className="mb-4 rounded-lg border border-slate-200 p-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600">GST Rate (%)</label>
                  <input
                    type="number"
            ...
            <tbody>
              {hsnSacList.map((h) => (
                <tr key={h.id} className="border-b border-slate-100">
                  <td className="py-2 font-medium text-slate-900">{h.code}</td>
                  <td className="py-2 text-slate-600">{h.description}</td>
                  <td className="py-2 text-slate-600 uppercase">{h.code_type}</td>
                  <td className="py-2 text-right font-medium">{h.gst_rate}%</td>
                  <td className="py-2">
                    <span className={`rounded-full px-2 py-0.5 text-xs ${h.is_active ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>
                      {h.is_active ? "Active" : "Inactive"}
                    </span>
                  </td>
                  <td className="py-2 text-right">
                    <button onClick={() => handleDeleteHsnSac(h.id)} className="text-xs text-red-500 hover:text-red-700">Delete</button>
                  </td>
                </tr>
              ))}
              {hsnSacList.length === 0 && (
          ...
          {showRegForm && (
            <div className="mb-4 rounded-lg border border-slate-200 p-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600">Trade Name</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600">State Code</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600">PAN</label>
                  <input
                    type="text"
                    value={regForm.pan}
                ...
                <div className="flex items-end">
                  <label className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-sm text-slate-600">Primary GSTIN</span>
                  </label>
                </div>
              </div>
              <button
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-slate-900">{r.gstin}</span>
                      {r.is_primary && (
                        <span className="rounded-full bg-brand-50 px-2 py-0.5 text-xs font-medium text-brand-700">Primary</span>
                      )}
                    </div>
                    <div className="mt-1 text-sm text-slate-600">{r.legal_name}</div>
                    {r.trade_name && <div className="text-sm text-slate-500">Trade: {r.trade_name}</div>}
                    <div className="mt-1 text-xs text-slate-400">State: {r.state_code} {r.pan && `| PAN: ${r.pan}`}</div>
                  </div>
                  <button onClick={() => handleDeleteRegistration(r.id)} className="text-xs text-red-500 hover:text-red-700">Delete</button>
                </div>
              </div>
            ))}
