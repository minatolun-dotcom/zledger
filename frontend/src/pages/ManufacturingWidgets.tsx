  ...
  const kpis: Record<string, React.ReactNode> = {
    boms: (
      <div className="flex items-center gap-2">
        <p className="text-lg font-bold tabular-nums text-slate-900 dark:text-[#f1f5f9]">{data.total_boms}</p>
        <span className="text-[11px] text-slate-500 dark:text-[#cbd5e1]">· {data.active_boms} active</span>
      </div>
    ),
    orders: (
      <div className="flex items-center gap-2">
        <p className="text-lg font-bold tabular-nums text-slate-900 dark:text-[#f1f5f9]">{totalOrders}</p>
    ...
    <div className={`${cardShell} p-4`}>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        {KPI_TILES.map((tile) => (
          <div
            key={tile.key}
            className="flex items-center gap-2.5 rounded-lg border border-slate-100 bg-white p-2.5 transition-colors hover:border-slate-200 dark:border-[#282832] dark:bg-[#1a1a24] dark:hover:border-[#33333f]"
          >
            <div className={`${iconTile} ${tile.iconBg}`}>
              {tile.icon}
            </div>
            <div className="min-w-0">
              <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-[#64748b]">{tile.label}</p>
              {kpis[tile.key]}
            </div>
          </div>
        ))}
