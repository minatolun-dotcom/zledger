 ...
 * Shared by the voucher sidebar, PartyDetailsPanel and the Sales/Purchase
 * party cards so outstanding always comes from the same source.
 */
export function usePartyOutstanding(
  ...
  }, [party?.id, party?.name, party?.party_type]);
