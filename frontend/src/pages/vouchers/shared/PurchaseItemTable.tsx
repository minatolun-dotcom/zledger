...
export default function PurchaseItemTable({
  ...
  return (
    <div
      ref={tableRef}
      className="rounded-lg border border-slate-200 dark:border-[#282832] bg-white dark:bg-[#16161f] overflow-x-auto"
    >
                ...
                >
                  ...
                  >
                    <select
                      ...
                      onChange={(e) =>
                        updateLine(rowIdx, {
                          is_rate_inclusive: e.target.value === "1",
                        })
                      }
                      className="w-full text-center border-0 bg-transparent text-sm text-slate-900 dark:text-[#f1f5f9] focus:outline-none focus:ring-0 p-1"
                    >
                      <option value="0">Excl</option>
      ...
      {/* Add Item button */}
      <div className="border-t-2 border-slate-200 dark:border-[#282832] bg-slate-50 dark:bg-[#1a1a24] px-2 py-2 text-center">
        <button
          type="button"
          onClick={() => {
            const updated = [...lines, emptyLine()];
