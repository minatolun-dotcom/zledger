    ...
    <div className="space-y-2">
      <div className="grid grid-cols-[1fr_auto_1fr] items-end gap-2">
        ...
        <div className="flex flex-col items-center gap-0.5 pb-1">
          <svg className="h-4 w-6 text-slate-300" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" />
          </svg>
          <div className="relative">
            <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[11px] font-medium text-slate-400">₹</span>
            <input
              type="number"
              min="0"
              step="0.01"
              ...
              className="w-28 rounded border border-slate-300 pl-5 pr-2 py-1 text-center text-sm font-semibold tabular-nums focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
              placeholder="0.00"
            />
          </div>
        </div>
      ...
      {amount > 0 && fromLedgerId && toLedgerId && (
        <div className="rounded bg-slate-50 px-2.5 py-1.5 text-[11px] text-slate-600 text-center">
          {ledgers.find((l) => l.id === fromLedgerId)?.name || "—"}
          <span className="mx-1 font-bold text-slate-400">→</span>
          <span className="rounded bg-brand-100 px-1.5 py-0.5 font-semibold text-brand-700 tabular-nums">
