...
export default function RoutingsTab({ canEdit }: Props) {
  ...
  const cols: SortableColumn<Routing>[] = [
    ...
    { id: "is_active", header: "Status", cell: ({ row: { original: r } }) => (
      <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${r.is_active ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400" : "bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400"}`}>
    ...
    cols.push({ id: "_actions", header: "", cell: ({ row: { original: r } }) => (
      <div className="flex gap-1 justify-end">
        <button onClick={() => {
          ...
          className="text-slate-400 hover:text-blue-500 dark:text-slate-500 dark:hover:text-blue-400" title="Edit">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-[#282832] dark:bg-[#16161f] space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400">Routing Name *</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400">Finished Item ID *</label>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <h5 className="text-xs font-semibold text-slate-600 dark:text-slate-400">Operations</h5>
              {canEdit && (
            ...
            {formOps.length === 0 && <p className="text-xs text-slate-400 dark:text-slate-500">No operations added yet.</p>}
            {formOps.map((op, idx) => (
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={resetForm} className="px-3 py-1.5 text-sm text-slate-600 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200">Cancel</button>
          </div>
        </form>
      )}
