...
export default function Tabs({ tabs, active, onChange, className = "", compact = false }: TabsProps) {
  ...
  return (
    <div
      ref={containerRef}
      role="tablist"
      className={`${compact ? "flex w-full" : "inline-flex max-w-full items-center"} gap-1 overflow-x-auto rounded-full bg-slate-200/70 dark:bg-[#12121a] p-1 ${className}`}
        ...
        return (
          <button
            ...
            className={`relative whitespace-nowrap rounded-full font-medium transition-all duration-150 flex items-center justify-center gap-1 ${
              compact
                ? "flex-none px-2 py-1.5 text-[13px] md:flex-1 md:min-w-0"
                : "flex-none px-3.5 py-1.5 text-sm gap-1.5"
            } ${
              isActive
