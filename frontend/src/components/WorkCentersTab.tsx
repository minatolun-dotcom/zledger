...
export default function WorkCentersTab({ canEdit }: Props) {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!(await showConfirm(`Delete work center "${name}"?`, { danger: true, confirmLabel: "Delete" }))) return;
    try {
      ...
      {canEdit && (
        <div className="flex items-center justify-between">
          <p className="text-sm text-slate-500 dark:text-[#94a3b8]">
            {centers.length} work center{centers.length !== 1 ? "s" : ""}
          </p>
          <button
            onClick={() => { setEditingId(null); setFormName(""); setFormDept(""); setFormCapacity("1"); setFormUnit("units/hr"); setFormRate("0"); setShowForm(true); }}
