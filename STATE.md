...
- **Current E2E status (2026-07-13, hermetic `zledger_test` @ `:9091`):**
  ...
  - **Caveats (env / dependency, not product bugs):**
    - `pdf-exports.spec.ts` describe is skipped — `pdf-parse@2.4.5` CJS class API crashes the Playwright Node worker on the generated PDFs; needs a robust parser.
    - `restore-e2e.spec.ts` "Full restore: execute" is skipped — it DROPs + recreates the live `zledger_test` DB, killing the `api_e2e` pool; needs a dedicated DB instance.
    - `web_e2e` (nginx) intermittently exits (255) under long runs; the runner's `docker start` fallback recovers it, but the container is fragile in this environment.
  - **Note:** live `zledger` (`:9090`) itself is broken (missing `company_activity` migration + the heartbeat 500). It was never the E2E target, but if anyone runs the app against live they'll hit 500s. Live stack should be rebuilt + migrated (`alembic upgrade head`) to clear this.
