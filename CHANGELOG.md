...
- **Draft → Pending → Posted approval lifecycle.** Vouchers can be saved as drafts (`status=draft`) that skip all book effects (stock, bill references) and are invisible to every report until approved. `submit`/`approve`/`reject` endpoints (accountant+ role) transition drafts; approving applies the book effects exactly once. Editing a draft keeps it a draft; editing a pending voucher invalidates its approval (must re-submit); a posted voucher can never be un-posted via edit (400 — cancel is the only way), preventing silent book mutilation.
