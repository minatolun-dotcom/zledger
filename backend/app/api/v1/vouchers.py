...
def bulk_delete_vouchers(
    ...
    for vid in payload.voucher_ids:
        ...
        if v.status == "posted":
            ...
            continue
        # Also remove the linked reversal voucher (if any) to avoid orphans
        if v.reversed_by_voucher_id:
            rev = db.get(Voucher, v.reversed_by_voucher_id)
            if rev and rev.company_id == company.id:
...
def cancel_voucher(
    ...
    Soft cancel with an explicit reversal voucher: the original is flipped to
    status=cancelled (excluded from every report) AND a linked reversal voucher
    with opposite entries is created (status=reversed, linked via
    original_voucher_id/reversed_by_voucher_id) so the audit trail matches
    TallyPrime. Stock entries are removed (inventory restored) and
    cancellation inside a closed financial year is blocked.
    """
    v = db.get(Voucher, voucher_id)
    if not v or v.company_id != company.id:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Voucher not found")
    ...
    if v.status == "draft":
        raise HTTPException(status.HTTP_400_BAD_REQUEST, detail="Draft vouchers cannot be cancelled; delete them instead")
    _check_fy_closed(db, company.id, v.voucher_date)
